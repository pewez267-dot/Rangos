...
/**
 * Permission handling for Fantastic Essentials.
 *
 * Relies on vanilla operator permission levels, which work on any server and are honoured by
 * permission mods that adjust a player's command permission level. The required level for each
 * node is configurable through the config file.
 */
public final class Permissions {
    ...
    public static boolean check(CommandSourceStack source, String node, int defaultLevel) {
        int level = com.pewez.fantasticessentials.config.Config.get().permissionLevel(node, defaultLevel);
        if (level <= 0) {
            return true;
        }
