package com.fantasticshortcuts.gui.widget;
...
/**
 * Scrollable, searchable single-column list widget styled to match the FantasticCrates /
 * FantasticSpawners family (dark rows, highlighted selection, slim scrollbar).
 *
...
public class ScrollSelector<T> extends AbstractWidget {
    ...
    protected void renderWidget(final GuiGraphics g, final int mouseX, final int mouseY, final float partialTick) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            final String trimmed = font.plainSubstrByWidth(name, this.width - 12);
            g.drawString(font, trimmed, getX() + 3, rowY + (this.rowHeight - 8) / 2, 14737632, false);
        }
