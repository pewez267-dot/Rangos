 ...
 * per-character gradient interpolation. Pure common code (no rendering), so it can be
 * reused by the admin GUI preview and the client nametag renderer alike.
 */
public final class NametagBuilder {
    ...
    /** Builds the full extra line, e.g. {@code "Lvl 23 VETERAN"}. */
    public static Component buildLine(NametagData data) {
        ...
        if (data.usePassStyle()) {
            ...
            // Legacy-formatted string (e.g. from Fantastic Ranks); the font renderer
            // interprets the embedded section-sign codes.
    ...
    /**
     * Convenience used when only a plain {@link ChatFormatting} accent is desired.
     */
