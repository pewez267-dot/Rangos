...
public class ServerTickHandler {
   ...
   private static void onServerTick(MinecraftServer server) {
      if ((long)server.method_3780() - lastCheckTick >= 20L) {
         lastCheckTick = (long)server.method_3780();
   ...
   private static boolean processRegularTokens(class_3222 player, IPlayerDiscovery discovery) {
      ...
      if (discovery.getRegularTokens() < maxTokens && gameTime >= discovery.getNextRegularTokenTime()) {
         ...
         BlockPopsMod.LOGGER
            .debug("Granted regular token to {}. Total: {}/{}", new Object[]{player.method_5477().getString(), discovery.getRegularTokens(), maxTokens});
         return true;
      } else {
         return false;
      }
