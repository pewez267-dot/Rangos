...
public class FigureBlockModel extends GeoModel<FigureBlockEntity> {
   ...
   public class_2960 getModelResource(FigureBlockEntity animatable) {
      ...
      if (figure != null) {
         ...
         return model != null && !GeckoLibCache.getBakedModels().containsKey(model) ? FALLBACK_MODEL : model;
      } else {
         return FALLBACK_MODEL;
      }
   }

   public class_2960 getTextureResource(FigureBlockEntity animatable) {
      FigureDefinition figure = animatable.getFigureDefinition();
      if (figure == null) {
         ...
         if (figure.getType() == FigureType.PLAYER && figure.getPlayerUUID() != null) {
            ...
            if (blockSnapshot != null && !blockSnapshot.isEmpty()) {
               return this.getSkinLocationFromSnapshot(figure, blockSnapshot);
            } else {
   ...
   public class_2960 getAnimationResource(FigureBlockEntity animatable) {
      FigureDefinition figure = animatable.getFigureDefinition();
      if (figure != null) {
         int skinIndex = animatable.getAlternativeSkinIndex();
         if (skinIndex > 0 && figure.hasAlternatives()) {
   ...
   public class_1921 getRenderType(FigureBlockEntity animatable, class_2960 texture) {
