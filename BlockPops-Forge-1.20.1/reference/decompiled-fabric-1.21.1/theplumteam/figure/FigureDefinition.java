...
public class FigureDefinition {
   ...
   @Nullable
   private final class_2960 poseAnimationPath;
   private final FigureType type;
   private final UUID playerUUID;
   private final List<FigureDefinition.AlternativeSkin> alternatives;
   private final PopBlockColor favoriteColor;
   @Nullable
   private final String authorUrl;
   private List<String> hiddenBones = Collections.emptyList();
   private List<FigureDefinition.ExtraTexture> extraTextures = Collections.emptyList();
   ...
   public FigureDefinition(
      String id, String name, class_2960 modelPath, class_2960 texturePath, class_2960 animationPath, List<FigureDefinition.AlternativeSkin> alternatives
   ) {
   ...
   public FigureDefinition(
      ...
      class_2960 animationPath,
      List<FigureDefinition.AlternativeSkin> alternatives,
   ...
   public FigureDefinition(
      ...
      class_2960 texturePath,
      class_2960 animationPath,
      @Nullable class_2960 poseAnimationPath,
      List<FigureDefinition.AlternativeSkin> alternatives,
      @Nullable String authorUrl
   ) {
      this.id = id;
   ...
   public static FigureDefinition fromJson(JsonObject json) {
      ...
      class_2960 modelPath = class_2960.method_12829(json.get("model").getAsString());
      class_2960 animationPath = class_2960.method_12829(json.get("animation").getAsString());
      class_2960 poseAnimationPath = json.has("pose_animation") ? class_2960.method_12829(json.get("pose_animation").getAsString()) : null;
      ...
      if ("player".equals(type)) {
         ...
         boolean isDefaultModel = modelPath != null && modelPath.method_12832().equals("geo/figure/box_figure_default.geo.json");
         ...
         class_2960 texturePath = class_2960.method_12829(json.get("texture").getAsString());
         List<FigureDefinition.AlternativeSkin> alternatives = new ArrayList<>();
         if (json.has("alternatives")) {
            ...
            for (int i = 0; i < alternativesArray.size(); i++) {
               ...
               alternatives.add(FigureDefinition.AlternativeSkin.fromJson(altJson));
            }
         }
         ...
         if (json.has("extra_textures")) {
            List<FigureDefinition.ExtraTexture> extras = new ArrayList<>();
            JsonArray extrasArray = json.getAsJsonArray("extra_textures");

            for (int i = 0; i < extrasArray.size(); i++) {
               extras.add(FigureDefinition.ExtraTexture.fromJson(extrasArray.get(i).getAsJsonObject()));
            }
   ...
   @Nullable
   public class_2960 getPoseAnimationPath() {
   ...
   public List<FigureDefinition.AlternativeSkin> getAlternatives() {
   ...
   public List<FigureDefinition.ExtraTexture> getExtraTextures() {
   ...
   public class_2960 getModelForSkinIndex(int skinIndex) {
      if (skinIndex > 0 && this.hasAlternatives()) {
         int altListIndex = skinIndex - 1;
         if (altListIndex < this.alternatives.size()) {
            class_2960 altModel = this.alternatives.get(altListIndex).model();
            if (altModel != null) {
               return altModel;
            }
         }
   ...
   public Set<String> getAllVariantBoneNames() {
      ...
      for (FigureDefinition.AlternativeSkin alt : this.alternatives) {
   ...
   public JsonObject toJson() {
      ...
      if (!this.extraTextures.isEmpty()) {
         ...
         for (FigureDefinition.ExtraTexture extra : this.extraTextures) {
   ...
   public static record AlternativeSkin(
      ...
      public static FigureDefinition.AlternativeSkin fromJson(JsonObject json) {
         String name = json.get("name").getAsString();
         class_2960 model = json.has("model") ? class_2960.method_12829(json.get("model").getAsString()) : null;
         class_2960 texture = class_2960.method_12829(json.get("texture").getAsString());
         ...
         return new FigureDefinition.AlternativeSkin(name, model, texture, hiddenBones, showBoxFace, boxFaceUV, scale);
      }
   }

   public static record ExtraTexture(class_2960 texture, List<String> bones) {
      public static FigureDefinition.ExtraTexture fromJson(JsonObject json) {
         class_2960 texture = class_2960.method_12829(json.get("texture").getAsString());
         List<String> bones = new ArrayList<>();
         ...
         return new FigureDefinition.ExtraTexture(texture, bones);
      }
   }
}
