package com.fantastickits.gui.widget;
...
/**
 * Scrollable, searchable single-column list widget shared by every Fantastic Kits
 * screen, styled to match the FantasticCrates / FantasticSpawners family (dark rows,
 * highlighted selection, slim right-hand scrollbar, optional item icon per row).
 *
 * @param <T> the type of element shown in the list
 */
public class ScrollSelector<T> extends AbstractWidget {
    ...
    public T getSelected() {
        return (this.selectedIndex >= 0 && this.selectedIndex < this.filtered.size())
                ? this.filtered.get(this.selectedIndex) : null;
    }
        ...
        for (int i = 0; i < rows; i++) {
            ...
            if (this.icon != null) {
                ...
                if (stack != null && !stack.isEmpty()) {
                    g.renderItem(stack, getX() + 1, rowY + (this.rowHeight - 16) / 2);
                }
                textX = getX() + 20;
            }
