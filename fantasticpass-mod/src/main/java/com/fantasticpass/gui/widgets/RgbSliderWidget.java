...
public class RgbSliderWidget extends AbstractWidget {
   ...
   protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
      ...
      int[] baseColors = new int[]{-43691, -11141291, -11167233};

      for (int k = 0; k < 3; k++) {
         int rowY = this.getY() + k * rowH;
         int midY = rowY + rowH / 2;
         graphics.drawString(Minecraft.getInstance().font, labels[k], this.getX(), midY - 4, baseColors[k], false);
         graphics.fill(this.trackX(), midY - 2, this.trackX() + this.trackWidth(), midY + 2, -14671832);
         graphics.renderOutline(this.trackX(), midY - 2, this.trackWidth(), 4, -16777216);
         ...
         graphics.renderOutline(knobX - 2, rowY + 2, 5, rowH - 4, -16777216);
         graphics.drawString(Minecraft.getInstance().font, String.valueOf(values[k]), this.trackX() + this.trackWidth() - 22, rowY + 1, -5592406, false);
      }
   }

   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      if (button == 0 && this.active && this.visible) {
         if (!(mouseX < (double)this.trackX())
            && !(mouseX > (double)(this.trackX() + this.trackWidth()))
            && !(mouseY < (double)this.getY())
            && !(mouseY > (double)(this.getY() + this.height))) {
            int channel = (int)((mouseY - (double)this.getY()) / (double)this.rowHeight());
