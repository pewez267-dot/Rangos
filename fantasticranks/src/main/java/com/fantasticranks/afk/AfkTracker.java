...
/**
 * Self-contained anti-AFK detector, independent of any other mod. Samples each online
 * player every {@code check_interval_ticks} for changes in block position, horizontal
 * rotation, and vertical rotation; explicit interactions also count as activity. A player
 * is considered active while the time since their last activity is below the threshold.
 *
...
public final class AfkTracker {
    ...
    public void serverTick(MinecraftServer server) {
        ...
        int interval = Math.max(1, RanksConfig.CHECK_INTERVAL_TICKS.get());
        if (serverTick % interval != 0L) {
            return;
        }

        double minRotation = RanksConfig.MIN_ROTATION_CHANGE_DEGREES.get();
    ...
    public boolean isActive(ServerPlayer player) {
        ...
        long thresholdTicks = (long) Math.max(1, RanksConfig.AFK_THRESHOLD_SECONDS.get()) * 20L;
