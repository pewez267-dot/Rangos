...
public final class MenuCommands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        register(dispatcher, "anvil", "essentials.command.anvil", "container.repair",
                (id, inv, player) -> new AnvilMenu(id, inv, ContainerLevelAccess.NULL));
        register(dispatcher, "cartographytable", "essentials.command.cartographytable", "container.cartography_table",
                (id, inv, player) -> new CartographyTableMenu(id, inv, ContainerLevelAccess.NULL));
        register(dispatcher, "craft", "essentials.command.craft", "container.crafting",
                (id, inv, player) -> new CraftingMenu(id, inv, ContainerLevelAccess.NULL));
        register(dispatcher, "enchanting", "essentials.command.enchanting", "container.enchant",
                (id, inv, player) -> new EnchantmentMenu(id, inv, ContainerLevelAccess.NULL));
        register(dispatcher, "grindstone", "essentials.command.grindstone", "container.grindstone_title",
                (id, inv, player) -> new GrindstoneMenu(id, inv, ContainerLevelAccess.NULL));
        register(dispatcher, "loom", "essentials.command.loom", "container.loom",
                (id, inv, player) -> new LoomMenu(id, inv, ContainerLevelAccess.NULL));
        register(dispatcher, "smithing", "essentials.command.smithing", "container.upgrade",
                (id, inv, player) -> new SmithingMenu(id, inv, ContainerLevelAccess.NULL));
        register(dispatcher, "stonecutter", "essentials.command.stonecutter", "container.stonecutter",
                (id, inv, player) -> new StonecutterMenu(id, inv, ContainerLevelAccess.NULL));

        // Workbench alias
        dispatcher.register(Commands.literal("workbench")
                .requires(Permissions.require("essentials.command.craft", 0))
        ...
        dispatcher.register(Commands.literal("enderchest")
                .requires(Permissions.require("essentials.command.enderchest", 0))
