...
public final class ParticleNames
{
    ...
    private static void put(final String id, final String es) {
        ParticleNames.ES.put(id, es);
    }
    
    public static String spanish(final String path) {
        if (path == null) {
            return "(?)";
        }
        final String name = ParticleNames.ES.get(path);
        if (name != null) {
            return name;
        }
