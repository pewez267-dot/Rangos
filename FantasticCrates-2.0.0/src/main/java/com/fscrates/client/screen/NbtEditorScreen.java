...
public class NbtEditorScreen extends Screen
{
    ...
    protected void init() {
        ...
        final int gap = 4;
        final int tabW = (this.panelWidth - 16 - 4 * (tabs.length - 1)) / tabs.length;
    ...
    private void initLore() {
        ...
        final char[] colors = new char[8];
        final String[] texts = new String[8];
        for (int i = 0; i < 8; ++i) {
        ...
        for (int j = 0; j < 8; ++j) {
            ...
            eb.setHint((Component)Component.literal("Línea de lore " + (idx + 1)));
            eb.setResponder(s -> {
                texts[idx] = s;
                sync.run();
                return;
