...
public final class RegistryLists
{
    ...
    public static List<ResourceLocation> effects() {
        final List<ResourceLocation> list = new ArrayList<ResourceLocation>(ForgeRegistries.MOB_EFFECTS.getKeys());
        list.sort(Comparator.comparing((Function<? super ResourceLocation, ? extends Comparable>)ResourceLocation::toString));
        return list;
    }
    
    public static String effectName(final ResourceLocation rl) {
        if (rl == null) {
            return "(efecto)";
        }
